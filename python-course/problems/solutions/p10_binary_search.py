"""
Reference solution for problems/p10_binary_search.py  —  Binary search                            [Medium · searching]

    ▶ run all   : python3 tools/grade.py --solutions problems
    ▶ run this  : python3 problems/solutions/p10_binary_search.py
...
problem = load_problem('problems/p10_binary_search.py')
exec(problem.SOLUTION, problem.__dict__)     # replace the stubs with the solution

if __name__ == "__main__":
    run_expressions('p10_binary_search' + " (reference solution)", problem.CASES, problem.__dict__)
    summary()
