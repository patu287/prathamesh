"""
Reference solution for problems/p18_sliding_window.py  —  Sliding window                          [Medium-Hard · windows]

    ▶ run all   : python3 tools/grade.py --solutions problems
    ▶ run this  : python3 problems/solutions/p18_sliding_window.py
...
problem = load_problem('problems/p18_sliding_window.py')
exec(problem.SOLUTION, problem.__dict__)     # replace the stubs with the solution

if __name__ == "__main__":
    run_expressions('p18_sliding_window' + " (reference solution)", problem.CASES, problem.__dict__)
    summary()
