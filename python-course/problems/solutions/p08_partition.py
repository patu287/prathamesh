"""
Reference solution for problems/p08_partition.py  —  Partitioning an array                  [Medium · two pointers]

    ▶ run all   : python3 tools/grade.py --solutions problems
    ▶ run this  : python3 problems/solutions/p08_partition.py
...
problem = load_problem('problems/p08_partition.py')
exec(problem.SOLUTION, problem.__dict__)     # replace the stubs with the solution

if __name__ == "__main__":
    run_expressions('p08_partition' + " (reference solution)", problem.CASES, problem.__dict__)
    summary()
