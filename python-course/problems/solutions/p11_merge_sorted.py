"""
Reference solution for problems/p11_merge_sorted.py  —  Merging sorted lists                     [Medium · two pointers]

    ▶ run all   : python3 tools/grade.py --solutions problems
    ▶ run this  : python3 problems/solutions/p11_merge_sorted.py
...
problem = load_problem('problems/p11_merge_sorted.py')
exec(problem.SOLUTION, problem.__dict__)     # replace the stubs with the solution

if __name__ == "__main__":
    run_expressions('p11_merge_sorted' + " (reference solution)", problem.CASES, problem.__dict__)
    summary()
